...
function show_matrice(){
	...
	document.getElementById('matrice').innerHTML +='Alignement 1:'+result[0]+'<br>';
	document.getElementById('matrice').innerHTML +='Alignement 2:'+result[1]+'<br><br>';
	document.getElementById('matrice').innerHTML +='Matrice séquences :'+ matseq+'<br>';
	document.getElementById('matrice').innerHTML +='Matrice scores :'+ matscore1+'<br>';
	document.getElementById('matrice').innerHTML +='Matrice chemins :'+ matchemin+'<br>';

	
	var matrice=document.getElementById("matrice2");
	...
	var matrice1=document.getElementById("matrice2").rows;//création des lignes
		...
		for (var j = 0; j < colonne.length ; j++) {
			...
			if (i>=2 && j===0){ 
				...
				for(var colonne in s1){
					...
					matrice2.rows[i].cells[j].innerHTML=s1[colonne];
					i++;		
				}
			};

			if (i===0 && j>=3) { //Remplir la première ligne à partir de la seconde case
				// console.log("on est dans le 2nd if");
				// console.log("tour",j)
				for (var ligne in s2) {
					// console.log("Ligne : "+ ligne + " = " + s2[ligne]);
					matrice2.rows[i].cells[j].innerHTML=s2[ligne];
					j++;
			...
			if (i>=1 && j===2){
				for (scoring in matscore1){
					matrice2.rows[i].cells[j].innerHTML=matscore1[scoring];
					j++;
					if(j%(taille2+1)==0){
						j=2;
						i++;
...
//Calcul des scores des matrices
function score(matscore1,matchemin,l1,l2,lengthseq,place){
	...
	//Autrement on calcul les scores
		...
		//Mettre le score obtenu dans la matrice de score
		matscore1[place]=maxiscore;
		//Remplissage de la matrice chemin suivant la matrice score
		if (maxiscore==(sumhor)){
			matchemin[place]=1;	} 
...
function alignment(matchemin,matscore1,s1,s2,len1,len2,lengthseq){
	...
	var align1=[];
	var align2=[];
	//Pour toute valeur dans la matrice de score
	//valmaxpos est sa place dans la matrice
	for (val in matscore1){
		valmaxpos=matscore1[val];
		valmax=Math.max(valmax,valmaxpos);
	}
	//Pour tout élément dans la matrice de score
